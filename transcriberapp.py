...
default_download_path = os.path.join(current_working_directory, "data/downloads")
default_output_path = os.path.join(current_working_directory, "data/podcast-transcriptions")


# look for a config file locally and if not exist then create one and auto-load it into the app
# - this way everything is built around a config file
...
if os.path.exists(feeds_file_path):
    print("Loading existing feeds config")
    rssList.load_feeds_from_json(feeds_file_path)
else:
    # create default feeds list
    print("Creating default feeds config")
    rssList.set_config_path(os.path.join(default_output_path, "podcasts-config"))
    rssList.set_cache_path(default_output_path)
    rssList.set_download_path(default_download_path)
    rssList.set_output_path(default_output_path)

    # create default folders
    rssList.make_dirs()

    rssList.feeds.append(RssFeed("The EvilTester Show", "https://feed.pod.co/the-evil-tester-show", ["testing"], "https://eviltester.com/show", ["https://eviltester.com"],"2025 04 01 00:00:01 UTC", "The Evil Tester Show - hosted by Alan Richardson. Occasional guest episodes. Show notes have comprehensive descriptions and resource links. Released as audio and video."))
    rssList.save_feeds_to_json(feeds_file_path)


# TODO: debug console print summary of rss feeds loaded
...
def reset_feed_to(rsslist_config):
    # API config
