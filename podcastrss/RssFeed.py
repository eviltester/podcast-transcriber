...
class RssFeed:
    ...
    def to_dict(self):
        """Convert the RssFeed object to a dictionary for JSON serialization."""
        return {
            'feedname': self.feedname,
            'feed_rss_url': self.feed_rss_url,
            'categories': self.categories,
            'homeUrl': self.homeUrl,
            'hrefs': self.hrefs,
            'earliestAutoDownloadDate': self.earliestAutoDownloadDate,
            # only output the config
            # 'new_podcast_episodes': self.new_podcast_episodes,
            # 'seen_podcast_episodes': self.seen_podcast_episodes,
            # 'seen_urls': list(self.seen_urls),  # Convert set to list
            # 'new_urls': list(self.new_urls),    # Convert set to list
            # 'cache_path': self.cache_path,
            'url_safe_feedname': self.url_safe_feedname,
            'description': self.description
        }

    @classmethod
    def from_dict(cls, data):
        """Create an RssFeed object from a dictionary loaded from JSON."""
        # Create instance with required parameters
        instance = cls(
            feedname=data['feedname'],
            feed_rss_url=data['feed_rss_url'],
            categories=data.get('categories'),
            homeUrl=data.get('homeUrl', ''),
            hrefs=data.get('hrefs'),
            earliestAutoDownloadDate=data.get('earliestAutoDownloadDate'),
            description=data.get('description', '')
        )

        # Set additional attributes that might have been modified after initialization
        # instance.new_podcast_episodes = data.get('new_podcast_episodes', [])
        # instance.seen_podcast_episodes = data.get('seen_podcast_episodes', [])
        # instance.seen_urls = set(data.get('seen_urls', []))  # Convert list back to set
        # instance.new_urls = set(data.get('new_urls', []))    # Convert list back to set
        # instance.cache_path = data.get('cache_path', '')

        return instance

    def save_to_json(self, filepath):
        """Save the RssFeed object to a JSON file."""
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(self.to_dict(), f, indent=2, ensure_ascii=False)
            print(f"RssFeed saved to {filepath}")
        except Exception as e:
            print(f"Error saving to JSON: {e}")

    @classmethod
    def load_from_json(cls, filepath):
        """Load an RssFeed object from a JSON file."""
        try:
