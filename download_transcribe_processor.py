...
class DownloadAndTranscribeProcessor:
    ...
    def download_transcribe_and_queue_for_summarization(self):
        ...
        while next_download != None:
            ...
            # a YouTube short would not have a download URL
            if next_download.url == "" or next_download.url is None:
                print("SKIPPING Processing - No download available: ", next_download.podcast_name, " - ", next_download.episode_title)
            else:
                ...
                try:
                    ...
                    print(f"Error downloading, skipping file {next_download.podcast_name} - {next_download.episode_title} - {e}")

                if inputAudioFile is not None:
